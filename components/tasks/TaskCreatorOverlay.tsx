    ...
    <>
      ...
      <Animated.View style={[styles.creatorOverlay, animatedOverlayStyle, hiddenStyle]}>
        <KeyboardAwareScrollView
          ...
          contentContainerStyle={[styles.sheetContent, { paddingTop: insets.top + spacing.sm }]}
        >
          {/* ── 1. Title + Subtasks glass card ── */}
          <View style={styles.glassCard}>
            ...
            {draftSubtasks.map((subtask) => (
              <View key={subtask.id}>
                <View style={styles.glassCardSeparator} />
                <View style={styles.creatorSubtaskItemRow}>
          ...
          <View style={styles.reminderCard}>
            <View style={styles.reminderHeaderRow}>
              ...
              >
                ...
                <Text style={styles.addReminderText}>Reminders</Text>
              </TouchableOpacity>
              <TouchableOpacity
          ...
          {/* ── 4. Action rail — inline in scroll ── */}
          <View style={styles.actionRailInline}>
            <TouchableOpacity
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                if (isCostEditing) {
                  ...
                  setTimeout(() => failureCostInputRef.current?.focus(), 100);
                }
              }}
              accessibilityRole="button"
            ...
            <View>
              ...
              >
                <Feather name="camera" size={18} color={isProofEnabled ? colors.bg : colors.textSubtle} />
              </TouchableOpacity>
            </View>
            ...
            >
              <Feather name="clock" size={18} color={timeBoundEnabled ? colors.bg : colors.textSubtle} />
            </TouchableOpacity>

            <TouchableOpacity
              style={[
                styles.optionIconButton,
                eventSyncEnabled && styles.optionIconButtonCalendarActive,
              ]}
              activeOpacity={0.8}
            ...
            >
              <Feather name="calendar" size={18} color={eventSyncEnabled ? colors.bg : colors.textSubtle} />
            </TouchableOpacity>
          ...
          {/* ── 5. Cost editor panel ── */}
          {isCostEditing && (
            <View style={styles.expandedPanelYellow}>
              <View style={styles.expandedPanelHeader}>
                <Text style={styles.expandedPanelTitle}>Failure cost</Text>
                <TouchableOpacity
                  style={styles.optionsPanelCloseBtn}
                  onPress={() => setIsCostEditing(false)}
                  activeOpacity={0.8}
                  accessibilityRole="button"
                  accessibilityLabel="Close cost panel"
                >
                  <Feather name="x" size={14} color={colors.textMuted} />
                </TouchableOpacity>
              </View>
              <View style={styles.costPanelInputRow}>
                <Text style={styles.costPanelCurrency}>{currencySymbol}</Text>
                <TextInput
                  ref={failureCostInputRef}
                  style={styles.costPanelInput}
              ...
              <View style={styles.costQuickSelectRow}>
                {[1, 5, 10, 20].map((amount) => (
                  <TouchableOpacity
                    key={amount}
                    style={styles.costQuickButton}
                    activeOpacity={0.8}
                    onPress={() => setFailureCostInput(String(amount))}
                  >
                    <Text style={styles.costQuickButtonText}>{currencySymbol}{amount}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
          ...
          {/* ── 6. Time-bound / Calendar expanded panels ── */}
          {expandedControl ? (
            <View style={[
              styles.expandedPanel,
              expandedControl === 'timeBound' ? styles.expandedPanelCyanBg : styles.expandedPanelPurpleBg,
            ]}>
              <View style={styles.expandedPanelHeader}>
                <Text style={styles.expandedPanelTitle}>
