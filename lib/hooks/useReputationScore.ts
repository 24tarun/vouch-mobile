...
async function fetchReputationScore(userId: string): Promise<ReputationScoreData> {
...
export function useReputationScore(userId: string | null | undefined) {
  return useQuery({
    queryKey: queryKeys.reputationScore(userId),
    queryFn: () => fetchReputationScore(userId!),
    enabled: !!userId,
    staleTime: 60_000,
    gcTime: 5 * 60_000,
  });
