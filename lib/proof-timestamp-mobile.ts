...
function formatProofTimestampParts(parts: ProofTimestampParts): string {
...
function parseIfdAsciiEntry(
  ...
  return readAscii(bytes, 0, count).replace(/\0+$/, '').trim() || null;
}
...
function extractProofTimestampTextFromJpegBuffer(buffer: ArrayBuffer): string | null {
...
function extractProofTimestampTextFromVideoBuffer(buffer: ArrayBuffer, uploaderTimeZone: string): string | null {
  const view = new DataView(buffer);
  const bytes = new Uint8Array(buffer);
...
function extractProofTimestampTextFromBuffer(
  mimeType: string,
  buffer: ArrayBuffer,
  uploaderTimeZone: string,
): string {
  const normalizedMime = (mimeType || '').toLowerCase();

  const extracted = normalizedMime === 'image/jpeg' || normalizedMime === 'image/jpg'
